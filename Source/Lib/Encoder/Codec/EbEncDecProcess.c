...
EbErrorType signal_derivation_enc_dec_kernel_oq(
    ...
    // Derive MD Exit TH
    if (pd_pass == PD_PASS_0)
        context_ptr->md_exit_th = (pcs_ptr->parent_pcs_ptr->slice_type == I_SLICE) ? 0 : 30;
    else if (pd_pass == PD_PASS_1)
    ...
    if (pd_pass == PD_PASS_0)
        ...
        if(enc_mode < ENC_M8)
            ...
            context_ptr->dc_cand_only_flag = !pcs_ptr->parent_pcs_ptr->is_used_as_reference_flag ? EB_TRUE : EB_FALSE;
#else
        context_ptr->dc_cand_only_flag = EB_FALSE;
#endif
                    ...
                    if (adaptive_md_cycles_red_ctrls->enabled) {
                        if (adaptive_md_cycles_red_ctrls->skip_nsq_th) {
                            ...
                            if (nsq_blk_geom->shape != PART_N) {
                                int8_t pred_depth_refinement = results_ptr->leaf_data_array[d1_blk_idx].pred_depth_refinement;
...
#if COST_BASED_PRED_ONLY
                    int64_t th_offset = 0;
                    if (context_ptr->depth_refinement_ctrls.enabled && context_ptr->depth_refinement_ctrls.use_pred_block_cost) {
                        ...
                        else if (context_ptr->md_local_blk_unit[blk_geom->sqi_mds].default_cost < cost_th_2) {
                            th_offset = -5;
                        }
                    }
#endif
